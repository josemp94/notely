...
## Qué está HECHO (a 19-ago-2026, commit ~d14e1fa)
...
- Lo siguiente barato y visible (ver hoja de ruta del doc de paridad): formatos de número y fechas con hora/rango, grupos del campo Estado, ancho de columna y color condicional, atajos de teclado, backlinks, y en el editor toggle heading y embeds.
...
## Build y DESPLIEGUE
1. `npm run build` para verificar (arregla todos los errores TS/lint) y `npm run check` (asserts sobre filtros y orden de vistas; amplíalo si tocas `src/lib/viewData.ts`).
