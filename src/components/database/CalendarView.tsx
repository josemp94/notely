  ...
  return (
    <div>
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <button onClick={prev} className="rounded px-2 py-1 text-sm hover:bg-[var(--border)]/30" aria-label="Mes anterior">
          ‹
        </button>
        <span className="font-display min-w-[9rem] text-center text-lg font-bold">
          {MONTHS[cursor.m]} {cursor.y}
        </span>
        <button onClick={next} className="rounded px-2 py-1 text-sm hover:bg-[var(--border)]/30" aria-label="Mes siguiente">
          ›
        </button>
              ...
              {d && (
                <>
                  <div className="mb-1 flex items-center justify-between">
                    ...
                    <button
                      onClick={() => addOn(key!)}
                      className="text-xs text-[var(--muted)] opacity-0 hover:text-brand group-hover:opacity-100"
                      title="Añadir aquí"
                    >
                      +
                    </button>
